...
namespace C1Monitoring
{
	...
	/// Description of Rate.
	/// </summary>
	public partial class frmClientApplication : MyFormPage
	{
		...
		public frmClientApplication(int Role_type)
		{
			//
			...
			if (Role_type == 2)
			{
				...
				btbSubmit1.Enabled = false;
				btnReset1.Enabled = false;
			}
			else if (Role_type == 1)
			{
				btnDel1.Enabled = false;
				btbSubmit1.Enabled = true;
...
//			}
		...
		public void DisplayGridviewClientApp()
		{
			...
			grdClientApp.Columns["ID_Client_App"].Visible = false;
			grdClientApp.Columns["APP_ID"].Visible = false;
			grdClientApp.Columns["Creator"].Visible = false;
			grdClientApp.Columns["CreateDate"].Visible = false;
...
//			cmbClient.DataSource = myBindingSourceClient1;
		...
		void BtnDel1Click(object sender, EventArgs e)
		{
			...
			if (MessageBox.Show("Are you sure you want to delete this record?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
		        {
		            dcPosition.DeleteOne1(grdClientApp.CurrentRow.Cells["ID_Client_App"].Value.ToString());
					DisplayGridview();
					Reset();
		        }	
		}
